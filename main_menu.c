...
void MenuInit(MenuSettings *menu, GlobalSettings *settings) {
  ...
  menu->btn_fsz = 20;
  menu->title_pos_x =
      MenuCalculateXPos(menu->title_text, settings->screen_width, menu->btn_fsz);
...
void MenuInitButtons(Btn btn[], GlobalSettings *settings, MenuSettings *menu) {
  ...
  for (int i = 0; i < btn_size; i++) {
    ...
    btn[i].x_pos =
        MenuCalculateXPos(btn[i].text, settings->screen_width, menu->btn_fsz);
    btn[i].y_pos =
...
int MenuCalculateYPos(int s_height, int step) { return (int)s_height / 16 * step; };
