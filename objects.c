...
void
ObjectsLoadTextures (ObjectTextures *texture)
{
  texture->full = LoadTexture ("./assets/Blocks/Block_full.png");
  texture->two_third
      = LoadTexture ("./assets/Blocks/Block_2_third_health.png");
  texture->one_third
      = LoadTexture ("./assets/Blocks/Block_1_third_health.png");
  texture->nead_death
      = LoadTexture ("./assets/Blocks/Block_near_death_health.png");
  return;
};

void
ObjectsUnloadTextures (ObjectTextures *texture)
{
...
void
ObjectsInit (int x, int y, DestroybleObject obj[x][y])
{
  for (int i = 0; i < x; i++)
    {
      for (int j = 0; j < y; j++)
...
Texture2D
GetTextureByHealth (int health, ObjectTextures *textures)
{
  switch (health)
    {
    case 1:
