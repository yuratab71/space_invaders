...
extern "C"
{
    ...
    int main(void)
    {
        ...
        objects_on_map = OBJECTS_ON_MAP;
        objects_in_block = OBJECTS_IN_BLOCK;
        ...
        while (!settings.should_close && !WindowShouldClose())
        {
            ...
            {
                if (settings.is_paused)
                    DrawText("|| PAUSE", settings.screen_width - 180, 50, 20, RAYWHITE);

                if (!settings.is_paused) // If game is not in pause mode do all calculations
                {
                    ...
                    {
                        enemy_projectile.pos = GameGetRandomEnemyPosition(enemy_on_x, enemy_on_y, enemies);
                    ...
                    for (int i = 0; i < objects_on_map; i++)
                    {
                        for (int j = 0; j < objects_in_block; j++)
                        {
                            if (des_objects[i][j].health != 0)
                                if (!player.can_shoot)
                                {
                                    if (CheckCollisionRecs(player.bullet.collider, des_objects[i][j].collider))
                                    {
                            ...
                            {
                                {
                                    if (CheckCollisionRecs(des_objects[i][j].collider, enemy_projectile.collider))
                                    {
                                        des_objects[i][j].health -= 1;
                                        e_settings.can_shoot = true;
                                    };
                    ...
                    for (int i = 0; i < enemy_on_x; i++)
                    {
                        ...
                        {
                            if (GameProcessCollisionBulletOnEnemy(&player, &enemies[i][j], &audio))
                            {
                    ...
                    GameProcessEnemyGridMovement(&e_settings, enemy_on_x, enemy_on_y, enemies);
                }; // end of not pause calculation and changes to game state
                // DRAWING SECTION
                //
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        DrawTexture(exp_textures.exp_1, animations[i].pos.x, animations[i].pos.y, WHITE);
                    }
                    else if (animations[i].timer > 1000.0f)
                    {
                        DrawTexture(exp_textures.exp_2, animations[i].pos.x, animations[i].pos.y, WHITE);
                    }
                    else if (animations[i].timer > 850.0f)
                    {
                        DrawTexture(exp_textures.exp_3, animations[i].pos.x, animations[i].pos.y, WHITE);
                    }
                    else if (animations[i].timer > 600.0f)
                    {
                        DrawTexture(exp_textures.exp_4, animations[i].pos.x, animations[i].pos.y, WHITE);
                    }
                    else if (animations[i].timer > 450.0f)
                    {
                        DrawTexture(exp_textures.exp_5, animations[i].pos.x, animations[i].pos.y, WHITE);
                    }
                ...
                DrawText(TextFormat("SCORE %d", player.score), 50, 50, 20, RAYWHITE);
                GameDrawPlayer(&player);
                GameDrawPlayerBullet(&player);

                if (!e_settings.can_shoot)
                {
                    DrawTextureEx(e_textures.projectile, enemy_projectile.pos, 180.0f, 1.0f, WHITE);
                };
                ...
                for (int i = 0; i < 3; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        if (des_objects[i][j].health != 0)
                        {
                            DrawTexture(GetTextureByHealth(des_objects[i][j].health, &obj_textures),
                                        des_objects[i][j].collider.x, des_objects[i][j].collider.y, WHITE);
                        };
                    };
                };
