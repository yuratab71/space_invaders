...
{
    background->texture = LoadTexture("./assets/PixelSpaceRage/PixelBackgroundSeamlessVertically.png");
    return;
};
...
void BackgroundInit(BackgroundSettings *background, GlobalSettings *settings)
{
    ...
    background->scale = BackgroundGetScale(settings->screen_width, background->texture.width);
    return;
};
...
{
    DrawTextureEx(bstn->texture, (Vector2){0, bstn->y_pos}, 0.0f, bstn->scale, WHITE);
    DrawTextureEx(bstn->texture, (Vector2){0, bstn->y_pos - bstn->texture.height * bstn->scale}, 0.0f, bstn->scale,
                  WHITE);
